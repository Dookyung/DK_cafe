...
Master 버전의 copy 또는 snapshot을 만들어 프로그램을 변경한 후 이를 Master 버전에 merging하여 수정사항을 반영하기 위해 사용

### How to create a new branch:

1. [Github] 해당 repository에서 `branch: master` 버튼 클릭하고 branch name을 입력
1. [Github] `Create branch` 박스를 클릭하고 키보드의 “Enter” 버튼 클릭
1. [RStudio] Git탭에서 `Pull` 버튼 클릭
1. [RStudio] Git탭의 `master` 버튼을 클릭 -> branch name을 선택하여 작업공간을
    master에서 new branch로 변경
...
1. [RStudio에서] 프로그램을 수정한 후 저장
1. Git탭에서 변경된 파일의 `Staged` 체크박스 체크
1. `Commit` 버튼 클릭
1. `Review changes`창이 나타나면 commit message를 입력
1. `Commit` 버튼 클릭 -> `Push` 버튼 클릭
1. [Github으로 이동] 해당 repository 상단에 "Your recently pushed branches:" 메시지 확인
1. Click `create a pull request`.
1. 두 버전건 충돌 없음 메시지 확인
1. Click on the button that says **Compare and pull request**.
1. Click the green **Merge pull request** button.
1. Click the green **Confirm merge** button.
1. Click the **Delete branch** button in the purple box.
